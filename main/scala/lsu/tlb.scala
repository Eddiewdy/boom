...
class NBDTLB(instruction: Boolean, lgMaxSize: Int, cfg: TLBConfig, numTLBPorts: Int = -1)(implicit edge: TLEdgeOut, p: Parameters) extends BoomModule()(p) {
  require(!instruction)
  
  // 如果没有指定端口数量，则使用默认的memWidth
  val tlbPorts = if (numTLBPorts > 0) numTLBPorts else memWidth
  
  val io = IO(new Bundle {
    val req = Flipped(Vec(tlbPorts, Decoupled(new TLBReq(lgMaxSize))))
    val miss_rdy = Output(Bool())
    val resp = Output(Vec(tlbPorts, new TLBResp))
  ...
  def widthMap[T <: Data](f: Int => T) = VecInit((0 until tlbPorts).map(f))
  ...
  val pmp = Seq.fill(tlbPorts) { Module(new PMPChecker(lgMaxSize)) }
  for (w <- 0 until tlbPorts) {

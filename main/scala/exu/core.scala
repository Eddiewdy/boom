...
//      ("I$ blocked",                        () => icache_blocked),
  ...
  // 初始化write_addr和write_data
  for (w <- 0 until exe_units.numIrfReaders) {
    event_counters.io.write_addr(w).valid := false.B
    event_counters.io.write_addr(w).bits := 0.U
    event_counters.io.write_data(w) := 0.U
  }
  ...
  for (w <- 0 until exe_units.length) {
    ...
    if (exe_unit.readsIrf) {
      ...
      when (uop.writeCounter && uop.ldst === 0.U) {
        ...
        event_counters.io.write_addr(iss_idx).bits := counter_idx
        event_counters.io.write_data(iss_idx) := rs1_data
        printf("writeCounter, pc: 0x%x, tag: %d, rs1: %d, data: 0x%x\n", uop.debug_pc, counter_idx, uop.lrs1, rs1_data)
      }
